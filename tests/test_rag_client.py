...
def test_elasticsearch_retriever_cloud_client_single_document(
    elasticsearch_cloud_test_index,
):
    elasticsearch = Elasticsearch(
    ...
    client = ElasticsearchRetrieverClient(
        elasticsearch=elasticsearch,
        embeddings=embeddings,
        index_name=elasticsearch_cloud_test_index,
    )
    sample_document = WebPage(
        url="https://www.example.com",
        created_at=datetime.now(),
    ...
    # create and assert writing working
    client.create_insert_webpage_document(sample_document)
    document_count = client.elasticsearch.count(index=elasticsearch_cloud_test_index)[
        "count"
    ]
